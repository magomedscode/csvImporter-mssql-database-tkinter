...
def import_csv():
    ...
    with open(file_path, 'r') as f:
        ...
        for row in reader:
            # neue leere liste "row_values" erstellt
            row_values = []
            for value in row:
                # wert zu row_values ohne zu teilen
...
def File_Manager():
    ...
    Label_of_filemanager = tk.Label(root2, text="List aller importierten CSVs", font=("Calibri", 16, "bold"),
                                    bg="#34495E", fg="white")
    ...
    try:
        ...
        label = tk.Label(root2, text="No CSV's imported", font=("Arial", 12), bg="#34495E", fg="white")
        # add padding to the label
        label.pack(padx=20, pady=10)
        print(e)
...
def search_csv():
    ...
    if searched_value != "":
        ...
        if matching_files != []:
            ...
            search_label = tk.Label(root3, text="Suchergebnisse werden hier angezeigt", font=("Helvetica", 12, "bold"),bg="#34495E", fg="white")
...
def account_creation():
    ...
    if email_signup == "" or password_signup == "":
        ...
        #hash passwort
        hashed_password = bcrypt.hashpw(password_signup.encode('utf-8'), bcrypt.gensalt())

        insert_statement_sql = f"INSERT INTO {table_name_signup} (email, password) VALUES (?, ?);"
        cursor = conn.cursor()

        #speichere hashed passwort statt wörter
        cursor.execute(insert_statement_sql, (email_signup, hashed_password.decode('utf-8')))
        cursor.commit()
...
def toggle_dashboard():
    ...
    if result:
        ...
        messagebox.showinfo("Angemeldet ","Willkommen")
